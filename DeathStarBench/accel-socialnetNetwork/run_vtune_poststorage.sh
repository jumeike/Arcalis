...
OUTDIR=vtune_poststorage_results

# Select binary and tag
if [ "$MODE" == "kernel" ]; then
    BIN="./poststorage_kernel_server"
    TAG="kernel"
elif [ "$MODE" == "dpdk" ]; then
    BIN="./poststorage_dpdk_server"
    TAG="dpdk"
else
...
sudo /opt/intel/oneapi/vtune/2025.4/bin64/vtune -collect $COLLECTION \
      -duration $DURATION \
      -target-pid $APP_PID \
      -result-dir $OUTDIR/poststorage_${TAG}.${SUFFIX} \
      --quiet
...
echo "Done. Results in: $OUTDIR/poststorage_${TAG}.${SUFFIX}"
