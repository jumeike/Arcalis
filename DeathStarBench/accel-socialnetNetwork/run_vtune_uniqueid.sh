...
OUTDIR=vtune_uniqueid_results

# Select binary and tag
if [ "$MODE" == "kernel" ]; then
    BIN="./uniqueid_kernel_server"
    TAG="kernel"
elif [ "$MODE" == "dpdk" ]; then
    BIN="./uniqueid_dpdk_server"
    TAG="dpdk"
else
...
sudo /opt/intel/oneapi/vtune/2025.4/bin64/vtune -collect $COLLECTION \
      -duration $DURATION \
      -target-pid $APP_PID \
      -result-dir $OUTDIR/uniqueid_${TAG}.${SUFFIX} \
      --quiet
...
echo "Done. Results in: $OUTDIR/uniqueid_${TAG}.${SUFFIX}"
