...
namespace {
#ifdef ENABLE_GEM5
bool pinCurrentThreadToCore(int core_id, const std::string& role) {
    if (core_id < 0) {
        return true;
    }
    ...
    if (rc != 0) {
        LOG(warning) << "Failed to pin " << role << " thread to core " << core_id;
        return false;
    }

    LOG(info) << "Pinned " << role << " thread to core " << core_id;
    return true;
}
#endif
} // namespace
            ...
            void* connectionContext)
{
...
#ifndef ENABLE_CEREBELLUM
    std::thread dispatch_thread;
    if (enable_core_split_) {
        pinCurrentThreadToCore(rpc_core_, "poststorage rpc");

        {
            std::lock_guard<std::mutex> lock(split_mutex_);
            split_request_ready_ = false;
            split_dispatch_done_ = false;
            split_stop_worker_ = false;
            split_dispatch_result_ = false;
        }

        dispatch_thread = std::thread([this]() {
            pinCurrentThreadToCore(business_core_, "poststorage business");
            for (;;) {
                std::unique_lock<std::mutex> lock(split_mutex_);
                split_cv_.wait(lock, [this]() {
                    return split_stop_worker_ || split_request_ready_;
                });

                if (split_stop_worker_ && !split_request_ready_) {
                    break;
                }
                ...
                split_dispatch_result_ = res;
                split_request_ready_ = false;
                split_dispatch_done_ = true;
    ...
    for (bool done = false; !done;) {
        ...
        if (enable_core_split_) {
            std::unique_lock<std::mutex> lock(split_mutex_);
            split_dispatch_done_ = false;
            split_request_ready_ = true;
            split_cv_.notify_one();

            split_cv_.wait(lock, [this]() { return split_dispatch_done_; });
            res = split_dispatch_result_;
        } else {
            res = callSWdispatch();
        }
...
#ifndef ENABLE_CEREBELLUM
    if (enable_core_split_ && dispatch_thread.joinable()) {
        {
            std::lock_guard<std::mutex> lock(split_mutex_);
            split_stop_worker_ = true;
            split_request_ready_ = false;
        }
        split_cv_.notify_one();
