...
void TServerSocket::listen() {
  ...
  if (ff_epoll_ctl(epfd, EPOLL_CTL_ADD, serverSocket_, &ev) != 0) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    // GlobalOutput.perror("TServerSocket::listen() ff_kevent() ", errno_copy);
    GlobalOutput.perror("TServerSocket::listen() ff_epoll_ctl() ", errno_copy);
    ff_close(kq_);
  ...
  // fprintf(stderr, "epfd value: %d, serverSocket_:%d in listen()\n", epfd, serverSocket_);
...
shared_ptr<TTransport> TServerSocket::acceptImpl() {
  ...
  int nevents = ff_epoll_wait(epfd,  events, DEFAULT_BACKLOG, 0);
  int i;
  if (nevents < 0) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TServerSocket::acceptImpl() ff_epoll_wait() error", errno_copy);
    // fprintf(stderr, "ff_epoll_wait error: %s\n", strerror(errno));
    // fprintf(stderr, "nevents: %d, epfd: %d, events_ addr: %p, DEFAULT_BACKLOG: %d\n", 
    //           nevents, epfd, (void*)events, DEFAULT_BACKLOG);
    throw TTransportException(TTransportException::UNKNOWN, "ff_epoll_wait()", errno_copy);
  }
  for (i = 0; i < nevents; ++i) {
        /* Handle new connect */
        if (events[i].data.fd == serverSocket_) {
          while (1) {
            ...
            THRIFT_SOCKET clientSocket = ff_accept(serverSocket_, NULL, NULL);
            fprintf(stderr, "New client connected!\n");
            // fprintf(stderr, "Debug: clientSocket after ff_accept: %d, events[%d].data.fd value: %d", clientSocket, i, events[i].data.fd);
            if (clientSocket < 0) {
                break;
            }
            ...
            ev.events  = EPOLLIN;
            if (ff_epoll_ctl(epfd, EPOLL_CTL_ADD, clientSocket, &ev) != 0) {
