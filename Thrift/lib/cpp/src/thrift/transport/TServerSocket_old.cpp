...
void TServerSocket::_setup_sockopts() {
  ...
  flags = 1;  // Set to non-blocking mode
...
shared_ptr<TTransport> TServerSocket::acceptImpl() {
  ...
  while (true) {
    ...
    int ret = ff_poll(fds, 2, accTimeout_);
    ...
    if (ret < 0) {
      ...
      if (interruptSockReader_ != THRIFT_INVALID_SOCKET && (fds[1].revents & THRIFT_POLLIN)) {
        ...
        if (-1 == ff_recv(interruptSockReader_, cast_sockopt(&buf), sizeof(int8_t), 0)) {
          // GlobalOutput.perror("TServerSocket::acceptImpl() recv() interrupt ",
          //                     THRIFT_GET_SOCKET_ERROR);
          GlobalOutput.perror("TServerSocket::acceptImpl() ff_recv() interrupt ",
                              THRIFT_GET_SOCKET_ERROR);
        }
      ...
      GlobalOutput("TServerSocket::acceptImpl() ff_poll 0");
  ...
  // if (kq == -1) {
  //   int errno_copy = THRIFT_GET_SOCKET_ERROR;
  //   GlobalOutput.perror("TServerSocket::acceptImpl() ff_kqueue() ", errno_copy);
  //   throw TTransportException(TTransportException::UNKNOWN, "ff_kqueue()", errno_copy);
  // }

  // struct kevent event;
  // struct kevent triggerEvent;
  // EV_SET(&event, serverSocket_, EVFILT_READ, EV_ADD, 0, 0, NULL);
  
  // if (ff_kevent(kq, &event, 1, NULL, 0, NULL) == -1) {
  //   int errno_copy = THRIFT_GET_SOCKET_ERROR;
  //   GlobalOutput.perror("TServerSocket::acceptImpl() ff_kevent() ", errno_copy);
  //   ff_close(kq);
  ...
  //   if (accTimeout_ >= 0) {
  //     timeout.tv_sec = accTimeout_ / 1000;
  //     timeout.tv_nsec = (accTimeout_ % 1000) * 1000000;
  //   }

  //   int ret = ff_kevent(kq, NULL, 0, &triggerEvent, 1, accTimeout_ >= 0 ? &timeout : NULL);
  ...
  //                               "TServerSocket::acceptImpl() timed out");
  //   } else {
  //     if (triggerEvent.ident == (uintptr_t)serverSocket_ && triggerEvent.filter == EVFILT_READ) {
  ...
  fprintf(stderr, "clientSocket: %d, serverSocket_:%d in TserverSocket::acceptImpl()\n", clientSocket, serverSocket_);
  ...
  fprintf(stderr,"created client and returning to TServerFramework");
  return client;
}
