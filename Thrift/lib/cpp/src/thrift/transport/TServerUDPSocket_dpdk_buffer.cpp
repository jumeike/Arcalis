...
#include <thrift/transport/BufferedTUDPSocket.h>
...
std::shared_ptr<TTransport> TServerUDPSocket::acceptImpl() {
  ...
  std::shared_ptr<BufferedTUDPSocket> client = createSocket(dpdkResources_->portId);
...
std::shared_ptr<BufferedTUDPSocket> TServerUDPSocket::createSocket(uint16_t portId) {
    // Create socket with shared DPDK resources
    auto socket = std::make_shared<BufferedTUDPSocket>(dpdkResources_);
