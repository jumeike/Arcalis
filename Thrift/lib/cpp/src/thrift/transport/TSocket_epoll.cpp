...
try_again:
  ...
  if (serverSocketInstance_) {
    ...
    int epfd = serverSocketInstance_->getEpfdDescriptor();
    struct epoll_event* events = serverSocketInstance_->getEpollEvents();
    ...
    // int nevents = 1;  // At least one event for the socket

    // Always monitor the socket for read events (already monitoring)
    // EV_SET(&eventlist[0], socket_, EVFILT_READ, EV_ADD | EV_ONESHOT, 0, 0, NULL);

    // If we have an interrupt listener, add it to the event list
    // if (interruptListener_) {
    //   EV_SET(&eventlist[0], static_cast<uintptr_t> (*(interruptListener_.get())), EVFILT_READ, EV_ADD | EV_ONESHOT, 0, 0, NULL);
    ...
    int ret = ff_epoll_wait(epfd,  events, 1024, 0);
    int i;
    // fprintf(stderr, "Debug: epfd value: %d, events[%d].data.fd value: %d\n", epfd, ret, events[ret].data.fd);
    if (ret < 0) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      if (errno_copy == THRIFT_EINTR && (retries++ < maxRecvRetries_)) {
        goto try_again;
      }
      GlobalOutput.perror("TSocket::read() ff_epoll_wait() wait ", errno_copy);
      throw TTransportException(TTransportException::UNKNOWN, "Unknown", errno_copy);
    } else if (ret == 0) {
      // Timeout
      // fprintf(stderr, "Debug: returning since ret=0\n");
      return got;
      //throw TTransportException(TTransportException::TIMED_OUT, "THRIFT_EAGAIN (timed out)");
    } else {
      for (i = 0; i < ret; ++i) {
        if (events[i].data.fd == socket_ && events[i].events == EPOLLIN) {
...
do_read:
  ...
  if (got > 0) {
    //fprintf(stderr, "Debug: read() operation was succesfull, returning got value: %d\n", got);
    return got;
  }
