...
try_again:
  ...
  if (serverSocketInstance_) {
  int kq = serverSocketInstance_->getKqueueDescriptor();
  const struct kevent* serverEvents = serverSocketInstance_->getEvents();
  ...
  // Prepare kevent structures for the interrupt listener
  //struct kevent eventlist[1];
  int nevents = 1024;  // At least one event for the socket

  // Always monitor the socket for read events (already monitoring)
  //EV_SET(&eventlist[0], socket_, EVFILT_READ, EV_ADD, 0, 0, NULL);
  //fprintf(stderr, "Debug: socket_ value: %d\n", socket_);
  // If we have an interrupt listener, add it to the event list
  //if (interruptListener_) {
    
  //  fprintf(stderr, "Debug: kq value: %d\n", kq);
  //  fprintf(stderr, "Debug: interruptListener_ value: %d\n",*(interruptListener_.get()));
    //EV_SET(eventlist, *(interruptListener_.get()), EVFILT_READ, EV_ADD | EV_ONESHOT, 0, 0, NULL);
    
    // Register the events we want to monitor
    //if (ff_kevent(kq, eventlist, nevents, NULL, 0, NULL) < 0) {
        //int errno_copy = THRIFT_GET_SOCKET_ERROR;
        //GlobalOutput.perror("TSocket::read() ff_kevent() register ", errno_copy);
        //throw TTransportException(TTransportException::UNKNOWN, "Failed to register events", errno_copy);
    //}
    //nevents = 1024;
  //}
  // Wait for events
  int ret = ff_kevent(kq, NULL, 0, const_cast<struct kevent*>(serverEvents), nevents, 
                      (recvTimeout_ > 0) ? &timeout : NULL);
  int i; 
  // fprintf(stderr, "Debug: kq value: %d, serverEvents[%d] value: %ld\n", kq, ret, serverEvents[ret].ident);
  if (ret < 0) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    if (errno_copy == THRIFT_EINTR && (retries++ < maxRecvRetries_)) {
      goto try_again;
    }
    GlobalOutput.perror("TSocket::read() ff_kevent() wait ", errno_copy);
    throw TTransportException(TTransportException::UNKNOWN, "Unknown", errno_copy);
  } else if (ret == 0) {
    // Timeout
    //fprintf(stderr, "Debug: filter value: %d\n", serverEvents[ret].filter);
    //if (retries++ < (maxRecvRetries_)) {
      //goto try_again;
    //}
    //fprintf(stderr, "returning since ret=0\n");
    return got;
    //throw TTransportException(TTransportException::TIMED_OUT, "THRIFT_EAGAIN (timed out)");
  } else {
    for (i = 0; i < ret; ++i) {
      if (interruptListener_ && serverEvents[i].ident == static_cast<uintptr_t> (*(interruptListener_.get()))) {
        // Interrupt event occurred
        throw TTransportException(TTransportException::INTERRUPTED, "Interrupted");
      } else if (serverEvents[i].ident == static_cast<uintptr_t> (socket_) && serverEvents[i].filter == EVFILT_READ) {
        // Data available to be read
        //fprintf(stderr, "Data available to be read\n");
        goto do_read;
      }
    }
    // If we get here, we didn't find our event, so try again
    goto try_again;
  }
  }
...
// New methods to access TServerSocket members
