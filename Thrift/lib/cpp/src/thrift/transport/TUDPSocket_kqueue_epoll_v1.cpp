...
TSocket::TSocket(THRIFT_SOCKET socket, TServerUDPSocket* serverSocketInstance, 
...
bool TSocket::peek() {
  ...
  if (interruptListener_) {
    for (int retries = 0;;) {
      ...
      if (ret < 0) {
        ...
        GlobalOutput.perror("TSocket::peek() ff_poll() ", errno_copy);
        throw TTransportException(TTransportException::UNKNOWN, "Unknown", errno_copy);
      } else if (ret > 0) {
  ...
  if (r == -1) {
    ...
    GlobalOutput.perror("TSocket::peek() ff_recv() " + getSocketInfo(), errno_copy);
...
void TSocket::openConnection(struct addrinfo* res) {
  ...
  if (socket_ == THRIFT_INVALID_SOCKET) {
    ...
    GlobalOutput.perror("TSocket::open() ff_socket() " + getSocketInfo(), errno_copy);
...
// Uses a low min RTO if asked to.
  ...
  if (connTimeout_ > 0) {
    ...
    if (-1 == ff_ioctl(socket_, FIONBIO, &onflag)) {
      ...
      GlobalOutput.perror("TSocket::open() ff_ioctl(FIONBIO, ON) " + getSocketInfo(), errno_copy);
    ...
    if (-1 == ff_ioctl(socket_, FIONBIO, &offflag)) {
      ...
      // GlobalOutput.perror("TSocket::open() THRIFT_FCNTL " + getSocketInfo(), errno_copy);
      // throw TTransportException(TTransportException::NOT_OPEN, "THRIFT_FCNTL() failed", errno_copy);
      GlobalOutput.perror("TSocket::open() ff_ioctl(FIONBIO, OFF) " + getSocketInfo(), errno_copy);
  ...
  if ((THRIFT_GET_SOCKET_ERROR != THRIFT_EINPROGRESS)
    ...
    GlobalOutput.perror("TSocket::open() ff_connect() " + getSocketInfo(), errno_copy);
  ...
  if (ret > 0) {
    ...
    if (ret2 == -1) {
      ...
      GlobalOutput.perror("TSocket::open() ff_getsockopt() " + getSocketInfo(), errno_copy);
    ...
    //                     val);
    GlobalOutput.perror("TSocket::open() error on socket (after ff_poll) " + getSocketInfo(),
                        val);
    ...
    GlobalOutput.perror("TSocket::open() ff_poll() " + getSocketInfo(), errno_copy);
  ...
  if (-1 == ff_ioctl(socket_, FIONBIO, &offflag)) {
    ...
    // GlobalOutput.perror("TSocket::open() THRIFT_FCNTL " + getSocketInfo(), errno_copy);
    // throw TTransportException(TTransportException::NOT_OPEN, "THRIFT_FCNTL() failed", errno_copy);
    GlobalOutput.perror("TSocket::open() ff_ioctl " + getSocketInfo(), errno_copy);
...
uint32_t TSocket::read(uint8_t* buf, uint32_t len) {
  ...
  if (serverSocketInstance_ && isUDP_) {
    ...
    if (numEvents < 0) {
      ...
      GlobalOutput.perror("TSocket::read() event wait error", errno_copy);
...
void TSocket::setLinger(bool on, int linger) {
  ...
  if (ret == -1) {
    ...
    GlobalOutput.perror("TSocket::setLinger() ff_setsockopt() " + getSocketInfo(), errno_copy);
  }
}
...
void TSocket::setNoDelay(bool noDelay) {
  ...
  if (ret == -1) {
    ...
    GlobalOutput.perror("TSocket::setNoDelay() ff_setsockopt() " + getSocketInfo(), errno_copy);
  }
}
...
void setGenericTimeout(THRIFT_SOCKET s, int timeout_ms, int optname) {
  ...
  if (ret == -1) {
    ...
    GlobalOutput.perror("TSocket::setGenericTimeout() ff_setsockopt() ", errno_copy);
  }
}
