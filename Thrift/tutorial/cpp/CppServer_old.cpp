...
int serve(void* arg) {
    TSimpleServer* server = static_cast<TSimpleServer*>(arg);
    if (server) {
        server->TServerFramework::serve();
        return 0;
    }
    return -1;  // Error: invalid server object
}
...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  TSimpleServer server(
    std::make_shared<CalculatorProcessor>(std::make_shared<CalculatorHandler>()),
    std::make_shared<TServerSocket>("192.168.1.1", 9090),
    std::make_shared<TBufferedTransportFactory>(),
    std::make_shared<TBinaryProtocolFactory>());
