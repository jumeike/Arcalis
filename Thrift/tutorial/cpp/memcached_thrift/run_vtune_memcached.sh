...
OUTDIR=vtune_memcached_results

# Select binary and tag
if [ "$MODE" == "kernel" ]; then
    BIN="./memcached_kernel_server"
    TAG="kernel"
elif [ "$MODE" == "dpdk" ]; then
    BIN="./memcached_dpdk_server"
    TAG="dpdk"
else
...
sudo /opt/intel/oneapi/vtune/2025.4/bin64/vtune -collect $COLLECTION \
      -duration $DURATION \
      -target-pid $APP_PID \
      -result-dir $OUTDIR/memcached_${TAG}.${SUFFIX} \
      --quiet
...
echo "Done. Results in: $OUTDIR/memcached_${TAG}.${SUFFIX}"
