...
//            pollThread_.join(); // Clean up any previous thread
//        }
...
//                responseCallback_(payload, payload_len);
//                responseCallback_ = nullptr;  // Clear after use
//            } 
            if (packetCallback_) {
...
//    bool response_received = false;  // Flag to track if response arrived
//    
//    {
//        std::lock_guard<std::mutex> lock(callback_mutex_);
//        responseCallback_ = [&](uint8_t* data, uint32_t len) {
//            response_received = true;
//            cv.notify_one();
//        };
//    } 
    
    // Start polling before sending
    //startPolling();
    ...
    // Wait here until response arrives
