...
public:
   MemcachedBusinessLogic()
       ...
       uint8_t* raw_recv_buf_ = new uint8_t[1024*1024 + 0x10];  // Extra space for alignment
       uint8_t* raw_resp_buf_ = new uint8_t[1024*1024 + 0x10];
   ...
   void handleReq(OpType op, size_t keylen, size_t valuelen = 0) {
       ...
       std::string key(reinterpret_cast<char*>(recv_buf_), keylen);
       ...
       if (op == GET) {
           ...
           if (success) {
               memcpy(resp_buf_, value.c_str(), value.length());
               logger.logLogicToRPC(resp_buf_, value.length());
               send_resp(GET, true, value.length());
               send_buf(resp_buf_);  // Send pointer to response buffer
           } else {
               send_resp(GET, false, 0);
           }
       } else { // SET
           std::string value(reinterpret_cast<char*>(recv_buf_ + keylen), valuelen);
           logger.logRPCToLogic(recv_buf_, keylen + valuelen);
...
MemcachedService_getRequest_args::~MemcachedService_getRequest_args() noexcept {
  ...
  public:
     ...
     void initCallbacks() {
        business_logic_->setCallbacks(
            ...
            [this](OpType op, bool success, size_t len) {
                if (op == GET) {
                    if (success) {
                        ...
                        current_response_ = "Key not found";
                    }
                } else { // SET
            ...
            [this](uint8_t* resp_buf) {
                memcpy(&current_response_[0], resp_buf, current_response_.length());
            }
        );
    }
   ...
   void getRequest(std::string& _return, const std::string& key) override {
       ...
       memcpy(recv_buffer_, key.c_str(), key.length());
       business_logic_->handleReq(GET, key.length());
   ...
   bool setRequest(const std::string& key, const std::string& value) override {
       ...
       memcpy(recv_buffer_, key.c_str(), key.length());
       memcpy(recv_buffer_ + key.length(), value.c_str(), value.length());

       business_logic_->handleReq(SET, key.length(), value.length());
       return set_success_;
   }
