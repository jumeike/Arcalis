...
//  };
    ...
    class MemcachedServiceHandler : virtual public MemcachedServiceIf {
    private:
        AppSharedMemory* app_shared_mem_;
        std::unique_ptr<MemcachedBusinessLogic> business_logic_;

    public:
        MemcachedServiceHandler() {
            auto& mgr = AppSharedMemoryManager::getInstance();
            app_shared_mem_ = mgr.initSharedMemory();
            if (!app_shared_mem_) {
                throw std::runtime_error("Failed to initialize app shared memory");
            }
            business_logic_ = std::make_unique<MemcachedBusinessLogic>();
        }

        void getRequest(std::string& _return, const std::string& key) override {
            pthread_mutex_lock(&app_shared_mem_->mutex);

            app_shared_mem_->msg.type = MemcachedMessage::GET;
            app_shared_mem_->msg.key = key;
            app_shared_mem_->has_data = true;

            bool success = business_logic_->handleGet(key, _return);
            app_shared_mem_->msg.success = success;

            pthread_mutex_unlock(&app_shared_mem_->mutex);
        }

        bool setRequest(const std::string& key, const std::string& value) override {
            pthread_mutex_lock(&app_shared_mem_->mutex);

            app_shared_mem_->msg.type = MemcachedMessage::SET;
            app_shared_mem_->msg.key = key;
            app_shared_mem_->msg.value = value;
            app_shared_mem_->has_data = true;

            bool result = business_logic_->handleSet(key, value);
            app_shared_mem_->msg.success = result;
...
public:
    ...
    bool handleGet(const std::string& key, std::string& value) {
        size_t value_length;
        uint32_t flags;
        memcached_return_t rc;
    ...
    bool handleSet(const std::string& key, const std::string& value) {
